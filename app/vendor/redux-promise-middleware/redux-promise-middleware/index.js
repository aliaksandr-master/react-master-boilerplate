// modified clone of https://github.com/pburtchaell/redux-promise-middleware
...
export default function promiseMiddleware(config = {}) {
  ...
  return ref => {
    ...
    return next => action => {
      ...
      const hash = meta && meta.selfID ? `${type}::${meta.selfID}` : null;

      if (hash) {
        activePromiseObjects
          .filter(({ hash: promiseObjectHash }) => hash === promiseObjectHash)
          .map((promiseObject) => {
            promiseObject.skipped = true;
          });
      }

      const promiseObject = { hash, skipped: false };

      activePromiseObjects.push(promiseObject);

      const cleanupPromiseObject = () => {
        activePromiseObjects = activePromiseObjects.filter((obj) => obj !== promiseObject);
      };
      ...
      const getAction = (newPayload, isRejected) => ({
        type: `${type}_${isRejected ? _REJECTED : _FULFILLED}${promiseObject.skipped ? `_${SKIPPED}` : ''}`,
